      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
        <div className="px-8 py-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between bg-slate-50/50 gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-1">
              Status-coded bookings overlaid on shift rules.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {[
              ...
              <div
                key={s.label}
                className="flex items-center gap-1.5 px-2.5 py-1 bg-white border border-slate-200 rounded-full shadow-sm"
              >
        ...
        <div className="p-8">
                  ...
                  <div className="h-8 bg-slate-100 rounded-xl relative overflow-hidden border border-slate-200/50 shadow-inner">
                    ...
                    {dayRules.map((rule) => (
                      <div
                        ...
                        style={{
                          ...
                          width: `${
                            timeStrToPct(rule.end) - timeStrToPct(rule.start)
                          }%`,
                        }}
                      />
                    ))}
                    ...
                    {dayBookings.map((booking) => {
                      ...
                      return (
                        <div
                          key={booking.id}
                          className={`absolute top-1 bottom-1 rounded-md shadow-sm border border-black/10 transition-transform group-hover:scale-y-110 ${getStatusColor(
                            booking.status
                          )}`}
                          style={{
                            left: `${startPct}%`,
          ...
          <div className="mt-8 p-6 bg-slate-50 rounded-3xl border border-dashed border-slate-200 flex items-start gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-slate-900 mb-1 uppercase tracking-tight">
                Timeline Legend
              </p>
              <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                The white spans indicate your <strong>Active Duty</strong>{" "}
                hours. Status-colored blocks represent booked client sessions.
                Overlaps or bookings outside active duty require administrative
                review.
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
        ...
        <div className="p-8 space-y-4">
          {rules.map((rule) => (
            <div
              key={rule.id}
              className="flex flex-col md:flex-row items-center gap-6 p-5 rounded-[2rem] border border-slate-100 bg-white hover:border-indigo-200 transition-all group shadow-sm hover:shadow-md"
            >
              <div className="w-full md:w-48">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setRules(
                      rules.map((r) =>
                        r.id === rule.id ? { ...r, day: e.target.value } : r
                      )
                    )
                  }
                >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setRules(
                          rules.map((r) =>
                            r.id === rule.id ? { ...r, end: e.target.value } : r
                          )
                        )
                      }
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border-0 rounded-xl font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 transition-all"
