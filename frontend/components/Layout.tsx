...
const Layout: React.FC<LayoutProps> = ({
  ...
  return (
    <div
      className={`flex h-screen bg-slate-50/50 overflow-hidden font-transition text-slate-900 ${fontClass}`}
    >
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-100 transform transition-all duration-500 lg:relative lg:translate-x-0 ${
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-6">
            <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
              ...
              <button className="w-full py-3 bg-white text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-50 transition-all">
                Upgrade Center
              </button>
            </div>
          </div>
        ...
        <header className="h-24 flex items-center justify-between px-8 bg-white/70 backdrop-blur-2xl border-b border-slate-100/50 z-40 sticky top-0">
          <div className="flex items-center gap-6">
            <button
              ...
              className="lg:hidden p-3 text-slate-500 bg-slate-50 rounded-2xl"
            >
              <Menu size={20} />
            </button>
                ...
                {isLocationOpen && (
                  <div className="absolute top-full left-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-6 py-4 text-left hover:bg-slate-50 transition-all ${
                        selectedLocationId === "ALL" ? "bg-indigo-50" : ""
                      }`}
                    >
                    ...
                    {locations.map((loc) => (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-6 py-4 text-left hover:bg-slate-50 border-t border-slate-50 transition-all ${
                          selectedLocationId === loc.id ? "bg-indigo-50" : ""
                        }`}
                      >
                        <div>
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2.5 bg-slate-50 border border-slate-100 rounded-xl group hover:bg-indigo-50 transition-all"
              >
                <Languages
                  size={18}
