...
export const ServiceModal: React.FC<ServiceModalProps> = ({
  ...
  const toggleRecurrence = (pattern: RecurrencePattern) => {
    ...
    if (current.includes(pattern)) {
      ...
      setFormData({
        ...formData,
        allowedRecurrence: current.filter((p) => p !== pattern),
      });
    } else {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            <h3 className="text-xl font-bold text-slate-900">
              {service ? "Update Service Protocol" : "Design New Offering"}
            </h3>
        ...
        <form
          ...
          className="p-10 space-y-8 max-h-[70vh] overflow-y-auto scrollbar-hide"
        >
          ...
          <div className="space-y-4">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-2">
              <RefreshCcw size={14} className="text-indigo-600" /> Authorized
              Recurrence Cycles
            </label>
        ...
        <div className="px-10 py-8 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-4">
          <button
            onClick={onClose}
            className="px-8 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs text-slate-400 hover:text-slate-600 hover:bg-white transition-all"
          >
            Discard
          </button>
          <button
            onClick={handleSubmit}
            className="flex items-center gap-2 bg-indigo-600 text-white px-10 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-200"
          >
            <Save size={18} />{" "}
            {service ? "Commit Update" : "Initialize Service"}
          </button>
        </div>
      </div>
    </div>
