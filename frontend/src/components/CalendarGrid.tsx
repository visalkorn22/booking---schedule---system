      ...
      <div className="px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          <div>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">
              Sync: {currentUser.timezone}
            </p>
          </div>
          <div className="flex bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <button className="p-3 hover:bg-slate-50 border-r border-slate-200 text-slate-400 hover:text-indigo-600 transition-colors">
              <ChevronLeft size={18} />
            </button>
            <button className="p-3 hover:bg-slate-50 text-slate-400 hover:text-indigo-600 transition-colors">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button className="px-4 py-2 bg-white text-indigo-600 rounded-lg text-xs font-black uppercase tracking-widest shadow-sm">
              Week
            </button>
            <button className="px-4 py-2 text-slate-500 rounded-lg text-xs font-black uppercase tracking-widest hover:text-indigo-600 transition-colors">
              Day
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto scrollbar-hide flex bg-slate-50/30">
        ...
        <div className="w-24 border-r border-slate-100 bg-white sticky left-0 z-30 flex-shrink-0">
          ...
          {HOURS.map((hour) => (
            <div
              key={hour}
              className="h-20 border-b border-slate-100 px-4 py-3 text-[10px] font-black text-slate-400 text-right uppercase tracking-tighter"
            >
        ...
        <div className="flex-1 grid grid-cols-7 min-w-[1200px]">
          {DAYS.map((day, dayIdx) => {
            ...
            const dayBookings = bookings.filter(
              (b) => new Date(b.startTime).getDate() === columnDate
            );

            return (
              <div
                key={day}
                className={`border-r border-slate-100 relative group/col`}
              >
                <div className="h-14 border-b border-slate-100 flex flex-col items-center justify-center bg-white/90 backdrop-blur sticky top-0 z-20">
                  ...
                  <p
                    className={`text-sm font-bold mt-0.5 ${
                      dayIdx === 2 ? "text-indigo-600" : "text-slate-900"
                    }`}
                  >
                    {columnDate}
                  </p>
                ...
                {HOURS.map((hour) => {
                  ...
                  return (
                    <div
                      key={hour}
                      className={`h-20 border-b border-slate-100/50 relative ${
                        isOffHours ? "bg-slate-100/40" : "bg-transparent"
                      }`}
                    />
                  );
                })}
                ...
                {dayBookings.map((booking) => {
                  ...
                  return (
                    <div
                      key={booking.id}
                      className="absolute inset-x-1.5 z-10 group/booking"
                      style={{ top: `${startY - bufferH}px` }}
                    >
                      {/* Prep Buffer */}
                      {bufferH > 0 && (
                        <div
                        ...
                        >
                          <span className="text-[8px] font-black uppercase tracking-[0.3em] opacity-0 group-hover/booking:opacity-100 transition-opacity">
                            Prep Zone
                          </span>
                        </div>
                      )}
                      ...
                      <div
                        ...
                        style={{
                          height: `${height}px`,
                          borderRadius: bufferH > 0 ? "0" : "1.5rem",
                        }}
                      >
                        ...
                        <div className="relative z-10 h-full flex flex-col">
                          ...
                          <p className="font-black text-xs leading-tight mb-1 truncate">
                            {service?.name}
                          </p>
                ...
                {dayIdx === 2 && (
                  <div
                    className="absolute left-0 right-0 h-0.5 bg-rose-500 z-40 flex items-center"
                    style={{ top: "350px" }}
                  >
                    <div className="w-2 h-2 rounded-full bg-rose-500 -ml-1 shadow-sm" />
                    <div className="px-2 py-0.5 bg-rose-500 text-white text-[8px] font-black rounded-r-lg uppercase tracking-widest">
                      Network Live
                    </div>
                  </div>
                )}
