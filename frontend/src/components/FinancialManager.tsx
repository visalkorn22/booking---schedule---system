...
export const FinancialManager: React.FC<FinancialManagerProps> = ({
  ...
  const totalDue = bookings.reduce(
    (sum, b) => sum + (b.totalPrice - b.paidAmount),
    0
  );
  ...
  const successRate =
    bookings.length > 0
      ? ((paidCount / bookings.length) * 100).toFixed(1)
      : "0.0";
  ...
  const handleExport = () => {
    ...
    const rows = filteredTransactions
      .map(
        (b) =>
          `${b.id},${b.id.includes("b1") ? "ABA" : "Stripe"},${b.totalPrice},${
            b.paidAmount
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm flex flex-col">
        <div className="px-10 py-8 border-b border-slate-100 bg-slate-50/30 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-1">
              Audit and track all financial settlements within the network.
            </p>
          </div>
                ...
                >
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-slate-900 font-mono text-xs uppercase tracking-tighter">
                  ...
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-3">
                      <span
                        className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.15em] border ${
        ...
        <div className="px-10 py-6 border-t border-slate-100 bg-slate-50/30 flex items-center justify-between">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            Showing {filteredTransactions.length} of {bookings.length} system
            entries
          </p>
          <div className="flex gap-2">
            <button
              className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black text-slate-400 uppercase disabled:opacity-50"
              disabled
            >
              Previous
            </button>
