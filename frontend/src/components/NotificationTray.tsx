      ...
      <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold mt-1">
            {notifications.filter((n) => !n.isRead).length} unread signals
            detected
          </p>
        </div>
      ...
      <div className="max-h-[450px] overflow-y-auto scrollbar-hide py-2">
        {notifications.length > 0 ? (
          notifications.map((n) => (
            <div
              key={n.id}
              className={`px-8 py-5 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-all cursor-pointer relative group ${
          ...
          <div className="p-20 text-center flex flex-col items-center">
            ...
            <p className="text-[10px] text-slate-300 mt-2">
              Your network is fully synchronized.
            </p>
          </div>
        )}
      </div>
