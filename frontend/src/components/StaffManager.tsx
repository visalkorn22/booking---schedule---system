...
import { type Staff, type Service } from "../../types";
...
export const StaffModal: React.FC<StaffModalProps> = ({
  ...
  const toggleService = (id: string) => {
    ...
    if (current.includes(id)) {
      setFormData({
        ...formData,
        assignedServices: current.filter((cid) => cid !== id),
      });
    } else {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              Manage credentials and service permissions.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              {formData.specialties?.map((s, i) => (
                <span
                  key={i}
                  className="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl text-xs font-bold flex items-center gap-2 border border-indigo-100"
                >
                  {s}
                  <button
              ...
              {availableServices.map((service) => (
                <button
                  ...
                  className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${
                    formData.assignedServices?.includes(service.id)
        ...
        <div className="px-10 py-8 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-4">
          <button
            onClick={onClose}
            className="px-8 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs text-slate-500 hover:bg-slate-100 transition-all"
          >
            Cancel
          </button>
          <button
            onClick={() => onSave(formData)}
            className="flex items-center gap-2 bg-indigo-600 text-white px-10 py-3.5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100"
          >
